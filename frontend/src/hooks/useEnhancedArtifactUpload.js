...
export default function useEnhancedArtifactUpload(type) {
  // Input type state (file, url, or text)
  ...
  // URL input state - using sourceUrl for consistency with API
  ...
  const validateForm = () => {
    ...
    if (inputType === 'file') {
      ...
      formData.sourceUrl = sanitizers.trim(sourceUrl);
    } else if (inputType === 'text') {
      formData.textContent = sanitizers.trim(textContent);
    }
    ...
    // Add input-specific validation rules
    if (inputType === 'file') {
      validationRules.file = [validators.required];
    } else if (inputType === 'url') {
      validationRules.sourceUrl = [
        validators.required,
        (value) => {
          try {
            new URL(value);
      ...
      validationRules.textContent = [
        validators.required,
        validators.minLength(10),
        (value) => value.length > 50000 ? 'Text content must be less than 50,000 characters' : null
      ];
    }
  ...
  const handleSubmit = async (e, onUpload) => {
    ...
    if (inputType === 'file') {
      ...
      uploadPayload.sourceUrl = formData.sourceUrl;
    } else if (inputType === 'text') {
      uploadPayload.textContent = formData.textContent;
    }
