...
export default function ProjectOutputUploadPopup({ projectId, projectName, onClose, onUpload }) {
  ...
  useEffect(() => {
    ...
    const loadOutputTypes = async () => {
      try {
        ...
        const types = await artifactApi.getArtifactTypes('output');
        setOutputTypes(types);

        // Set default output type if available
        if (types && types.length > 0) {
          setOutputType(types[0].id);
        }
      } catch (err) {

        setError('Failed to load output types. Please try again.');
      } finally {
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      // Use the file name as the default output name if no name is set
      if (!name) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name.trim()) {
      setError('Please provide a name for the output');
      return;
    }
    ...
    try {
      ...
      setError('Failed to upload output. Please try again.');
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-semibold text-gray-800">Add Project Output for {projectName}</h2>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4">
            <label htmlFor="name" className="block text-gray-700 mb-2">Output Name</label>
            <input
              id="name"
              type="text"
              value={name}
          ...
          <div className="mb-4">
            <label htmlFor="outputType" className="block text-gray-700 mb-2">Output Type</label>
            <select
              id="outputType"
              value={outputType}
          ...
          <div className="mb-6">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-purple"
              placeholder="Brief description of this project output"
              rows="3"
            />
          </div>
          ...
          <div className="flex justify-end space-x-3">
            ...
            >
              ...
              {isUploading ? 'Uploading...' : 'Add Output'}
            </button>
          </div>
        </form>
      </div>
