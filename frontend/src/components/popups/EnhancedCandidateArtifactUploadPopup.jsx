...
export default function EnhancedCandidateArtifactUploadPopup({ candidateId, candidateName, onClose, onSuccess }) {
  const {
    inputType,
    setInputType,
    file,
    ...
    handleSubmit,
    handleUploadAnother
  } = useEnhancedArtifactUpload('candidate');
  ...
  const handleUpload = async (uploadPayload) => {
    ...
    const result = await candidateApi.addCandidateArtifact(
      candidateId,
      artifactData,
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-semibold text-gray-800">
            {uploadSuccess ? 'Upload Complete' : `Upload Artifact for ${candidateName}`}
          </h2>
          <button 
            onClick={onClose} 
        ...
        {uploadSuccess ? (
          <div className="p-6 text-center">
            <div className="mb-6">
              ...
              <p className="text-gray-600">The artifact has been added to {candidateName}&apos;s profile.</p>
            </div>
